...
{
    // current step of the ray march
    float3 currentPoint = startPoint;
    intensity = 1.0;
    float distance = 0.0;
    for (int i = 0; i < recursionDepth; i+=1)
    {
        // calculate voronoi noise for current point
        float2 voronoi;
        if (rejected)
        {
            voronoi = VoronoiNoise3D(rejection(currentPoint, normal));
        }
        else
        {
            voronoi = VoronoiNoise3D(currentPoint);
        }
        // estimate value at this point
        float value = 1.0 - clamp(voronoi.y + 1.0 - border, 0.0, 1.0);
        //move current point along view axis
        currentPoint -= 0.5 * voronoi.y * dir;
        // store distance travelled so far
        distance += 0.5 * voronoi.y;
        // reduce output value the further it travels
        intensity = 1.0 - clamp(distance * fade, 0.0, 1.0);
        // if intensity gets really low, stop calculating
        if (intensity < 0.001 || value > 0.0)
        {
            return;
        }
